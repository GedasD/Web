...
<html>
    ...
    <!-- HEADER -->	
      ...
      <nav class="melyna padding">
		...
		<div class="section raudona text-center">
			<h1 class="text-white margin-bottom-0 text-size-50 text-thin text-line-height-1" style="color: white">Prisijungimas</h1>
		</div>
	
	
		 <!-- Log-in form -->
		<div class="s-12 m-12 l-4 pilka" id="registracija">
              <h4 class="text-uppercase text-strong form-name">Prisijunkite</h4>
				<form class="customform black-text" method="post" action="Login.php">
					<div class="s-12 m-12">
					  <input title="Your name"  name="LogName" class="border-radius" placeholder="Vardas" required  type="text" />
					</div>
					<div class="s-12 m-12 form-field-spacing">
					  <input title="Your password"  name="LogPassword" class="border-radius" placeholder="Slaptažodis" required  type="password" />
					</div>
					<div class="s-12"><button class=" form-butt"  type="submit" id="lol">Jungtis</button></div>
				</form>
		</div>
		<?php
		if(isset($_SESSION['vart_num'])){
			echo $_SESSION['vart_num'];
	...
	<script>
		...
		var error = getUrlParameter('error');
		 ...
		 alert('Vartotojo vardas arba slaptazodis yra neteisingas');
		 }
